...
export const hook = <Controller extends string, MethodName extends string>(
  fetcher: EventRequest,
  controller: Controller,
...
export const methodHook = <Controller extends string, MethodName extends string>(
  fetcher: EventRequest,
  controller: Controller,
  methodName: MethodName
...
export const Hooker = <Api>(fetcher: EventRequest): HookApi<Api> => {
  type HookerResult = HookApi<Api>;
  return new Proxy({} as HookerResult, {
    get: <Controller extends keyof HookerResult & string>(
      _: HookerResult,
      controller: Controller
    ): HookerResult[Controller] =>
      new Proxy({} as HookerResult[Controller], {
        get: <MethodName extends keyof HookerResult[Controller] & string>(
